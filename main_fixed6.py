...
def receive_a2s_response(sock, first_data=None):
    ...
    if first_data[:4] != b"\xff\xff\xff\xfe":
        return first_data
    ...
    while True:
        ...
        #
        # Example: 0x21 means packet 2 of 1? No:
        # packet_number = 2, total_packets = 1.
        # This is the format used by GoldSrc query implementations.
...
def query_players(sock, target, legacy=False):
    ...
    For old GoldSrc servers, try the original:
        FF FF FF FF "players"
    query first (no NUL terminator).

    Then fall back to the challenge-based A2S_PLAYER protocol.
    """
        ...
        if data is not None:
            if data[:5] == b"\xff\xff\xff\xff\x44":
                try:
                    ...
                    return [], "Legacy player parse error: {}".format(e)
    ...
    if (
        ...
        sock.sendto(
            b"\xff\xff\xff\xff" + b"\x55" + challenge,
            target
        )
...
def query_rules(sock, target, legacy=False):
    def receive():
        data, addr = sock.recvfrom(65535)
        if data[:4] == b"\xff\xff\xff\xfe":
            data = receive_a2s_response(sock, data)
    ...
    # GoldSrc/HLDS commonly accepts the original direct rules query
    # even when A2S_INFO replies with the newer 0x49 format.
    # Game-State-style scanners use the resulting 0x45 rules packet.
    #
    # IMPORTANT: "rules" has NO NUL terminator.
    sock.sendto(b"\xff\xff\xff\xff" + b"rules", target)
    try:
        data = receive()
    except Exception:
        data = None

    if data is not None and data[:5] == b"\xff\xff\xff\xff\x45":
        try:
            return parse_rules(data), None
        except Exception as e:
            return [], "Legacy rules parse error: {}".format(e)

    # If direct GoldSrc rules are not supported, fall back to
    # the challenge-based A2S_RULES protocol.
    sock.sendto(b"\xff\xff\xff\xff" + b"\x57", target)
    try:
        data = receive()
    except socket.timeout:
        return [], "Rules query timed out: server answered neither direct GoldSrc rules nor A2S_RULES challenge"
    except Exception as e:
        return [], "Rules split-packet error: {}".format(e)

    if not (len(data) >= 9 and data[:4] == b"\xff\xff\xff\xff" and data[4] == 0x41):
        return [], "Server did not return rules challenge"

    challenge = data[5:9]

    # GoldSrc/HLDS servers can enforce max_queries_sec=1.
    # Do not send the challenge response immediately after
    # the challenge packet; wait for the next query window.
    time.sleep(1.15)

    sock.sendto(b"\xff\xff\xff\xff" + b"\x56" + challenge, target)
...
def query_server(host, port):
    ...
    try:
        ...
        players, player_error = query_players(
            sock,
            target,
            legacy=(info.get("response_type") == "0x6D")
        )
        ...
        # ====================================================
        #
        # Many GoldSrc servers rate-limit query packets.
        # The player query has just completed, so give the
        # server a short gap before asking for rules.
        # This is especially important on servers exposing
        # max_queries_sec / max_queries_sec_global.
        #
        time.sleep(1.25)
