...
def query_players(sock, target, legacy=False):
    ...
    if legacy:
        ...
        if data is not None:
            if data[:5] == b"\xff\xff\xff\xff\x44":
                try:
                    return parse_players(data), None, challenge, None
                except Exception as e:
    ...
    try:
        ...
        return [], "Player split-packet error: {}".format(e), challenge, None

    if data[:5] == b"\xff\xff\xff\xff\x44":
        try:
            return parse_players(data), None
        except Exception as e:
            return [], "Player parse error: {}".format(e), challenge, None
    ...
    if (
        ...
        if data[:5] != b"\xff\xff\xff\xff\x44":
            return [], "Unexpected A2S_PLAYER response", None, challenge
...
def query_rules(sock, target, legacy=False, cached_challenge=None):
    ...
    # Modern GoldSrc/HLDS: reuse the challenge already obtained by
    # A2S_PLAYER. This avoids another challenge request and is important
    # for servers using max_queries_sec=1.
    if cached_challenge is not None:
        time.sleep(1.15)
        sock.sendto(b"\xff\xff\xff\xff" + b"\x56" + cached_challenge, target)
        try:
            data = receive()
        except socket.timeout:
    ...
    # No cached challenge: request one using the A2S_RULES -1 form.
    sock.sendto(b"\xff\xff\xff\xff" + b"\x56" + b"\xff\xff\xff\xff", target)
    try:
        data = receive()
    except socket.timeout:
        return [], "Rules challenge timed out"
    except Exception as e:
        return [], "Rules challenge error: {}".format(e)
...
def query_server(host, port):
    ...
    try:
        ...
        players, player_error, player_challenge = query_players(
            sock,
            target,
        ...
        rules, rules_error = query_rules(
            ...
            target,
            legacy=(info.get("response_type") == "0x6D"),
            cached_challenge=player_challenge
        )
