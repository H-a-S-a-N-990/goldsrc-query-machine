...
def query_rules(sock, target, legacy=False):
    """
    Query server rules using the standard A2S_RULES protocol.

    A2S_RULES request:
        FF FF FF FF 56 FF FF FF FF

    The -1 challenge tells the server to return an A2S challenge (0x41).
    Then the returned challenge is sent with 0x56.
    """
    ...
    # --------------------------------------------------------
    # Standard A2S_RULES challenge request
    # --------------------------------------------------------
    request = (
        b"\\xff\\xff\\xff\\xff"
        + b"\\x56"
    ...
    if not (
        ...
        return [], "Server did not return A2S_RULES challenge"

    challenge = data[5:9]

    # The target server advertises max_queries_sec=1.
    # Give it a full query window before sending the actual rules query.
    time.sleep(1.15)
    ...
    if data[:5] != b"\\xff\\xff\\xff\\xff\\x45":
        return [], "Unexpected A2S_RULES response"
